...
namespace SysCall
{
    auto GetSyscallIndex(std::string ModuleName, std::string SyscallFunctionName, void* Function) -> bool
    {
        ...
        if (!ModuleBaseAddress)
            ModuleBaseAddress = LoadLibraryA(ModuleName.c_str());

        if (!ModuleBaseAddress)
            return false;

        auto GetFunctionAddress = GetProcAddress(ModuleBaseAddress, SyscallFunctionName.c_str());
    ...
    VOID SendMouseEventEx(DWORD dwFlags, DWORD dx, DWORD dy, DWORD dwData, ULONG_PTR dwExtraInfo)
    {
